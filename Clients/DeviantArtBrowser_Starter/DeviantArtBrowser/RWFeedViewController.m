...
// Below this comment was all MFS
...
- (void)setImageForCell:(RWImageCell *)cell item:(RSSItem *)item {
  ...
  //mediaThumbnails are generally ordered by size, so get the second mediaThumbnail, which is a "medium" sized image
...
- (void)setTitleForCell:(RWBasicCellTableViewCell *)cell item:(RSSItem *)item {
  NSString *title = item.title ?: NSLocalizedString(@"[No Title]", nil);
  [cell.titleLabel setText:title];
}

-(void)setSubtitleForCell:(RWBasicCellTableViewCell *)cell item:(RSSItem *)item {
  NSString *subtitle = item.mediaText ?: item.mediaDescription;
  
  //Some subtitles can be really long, so only display the first 200 characters
...
- (CGFloat)calculateHeightForConfiguredSizingCell:(UITableViewCell *)sizingCell {
  ...
  return size.height +1.0f; //For cell sperator height
}

#pragma mark - EstimationHeight
